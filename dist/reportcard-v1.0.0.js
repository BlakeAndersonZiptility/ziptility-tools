/* Ziptility Utility Health Report Card reportcard-v1.0.0 (f4ae84ae846cecc7efa9b0a5bd0e8f8702d332b2) https://github.com/BlakeAndersonZiptility/ziptility-tools */
(()=>{var Y=`/* Utility Health Report Card: DS 4.0 reskin. Token block is copied 1:1
...
#ziptility-report-card .zrc-wheel-section svg{ display:block; }
...
/* ---------- C2: the Practical Grade lead, shown first and larger when
   two or more critical dimensions cap the composite (config.js
   PRACTICAL_CAP_THRESHOLD) ---------- */
#ziptility-report-card .zrc-practical-lead{ margin:0 0 18px; }
#ziptility-report-card .zrc-practical-grade-big{
  display:inline-flex; flex-direction:column; align-items:center; gap:2px;
  border-radius:var(--radius-lg); padding:16px 30px; margin-bottom:14px;
}
#ziptility-report-card .zrc-practical-letter{ font-family:var(--font-sans); font-weight:900; font-size:52px; line-height:1; }
#ziptility-report-card .zrc-practical-label{ font-family:var(--font-ui); font-size:13px; font-weight:700; text-transform:uppercase; letter-spacing:.04em; }
#ziptility-report-card .zrc-practical-explain{ font-size:16.5px; line-height:1.55; color:var(--n700); margin:0; max-width:640px; }
...
Each dimension's 'A \u2014 Thriving' column is the organization's future story. Capacity-building interventions that move a utility from F toward A should be evaluated by whether they add resources or impose requirements. This is the additive-versus-subtractive test described in the accompanying white paper \xA74 and \xA78.`}};var O=["T5","T7","T8","M3","M8","F1","F2"],K=["F"];var F={portalId:"4938013",formId:""};var D={F:0,D:1,C:2,B:3,A:4},w=["F","D","C","B","A"],S=["T","M","F"],k={T:"Technical",M:"Managerial",F:"Financial"},$=4;function I(e){return Object.prototype.hasOwnProperty.call(D,e)}function X(e){if(e==null||!isFinite(e))return null;let t=Math.round(e);return w[Math.max(0,Math.min(w.length-1,t))]||null}function ee(){return new Set(O)}function Re(e,t,n){let i=t.filter(o=>o.leg===n),a=i.map(o=>e[o.id]).filter(o=>I(o)).map(o=>D[o]);if(!a.length)return{leg:n,value:null,grade:null,answered:0,total:i.length};let s=a.reduce((o,c)=>o+c,0)/a.length;return{leg:n,value:s,grade:X(s),answered:a.length,total:i.length}}function We(e){let t=e.map(i=>i.value).filter(i=>i!=null&&isFinite(i));if(!t.length)return{value:null,grade:null,legsCounted:0};let n=t.reduce((i,a)=>i+a,0)/t.length;return{value:n,grade:X(n),legsCounted:t.length}}function Se(e,t){let n=ee(),i=new Set(K);return t.filter(a=>n.has(a.id)&&i.has(e[a.id])).map(a=>({id:a.id,name:a.name,leg:a.leg,grade:e[a.id]}))}function Ee(e,t){if(!(t.length>=2)||e.grade==null)return{capped:!1,grade:e.grade,descriptiveGrade:e.grade,flagCount:t.length};let i=D["D"]<D[e.grade];return{capped:i,grade:i?"D":e.grade,descriptiveGrade:e.grade,flagCount:t.length}}function Pe(e,t){let n=ee();return t.map((i,a)=>({d:i,index:a})).filter(({d:i})=>I(e[i.id])&&D[e[i.id]]<$).map(({d:i,index:a})=>{let s=D[e[i.id]],o=$-s,c=n.has(i.id);return{id:i.id,name:i.name,leg:i.leg,current:e[i.id],target:w[s+1],isRedLine:c,score:o*(c?2:1),index:a}}).sort((i,a)=>a.score-i.score||i.index-a.index).slice(0,3)}function N(e,t){if(!I(t))return null;let n=D[t];return n>=$?null:e.actions[t+"->"+w[n+1]]||null}function P(e,t){let n=S.map(s=>Re(e,t,s)),i=We(n),a=Se(e,t);return{legAverages:n,overall:i,flags:a,practical:Ee(i,a),oneRungUp:Pe(e,t),answered:t.filter(s=>I(e[s.id])).length,total:t.length,complete:t.every(s=>I(e[s.id]))}}var te="zip-reportcard-v1";function G(e){return e!==null&&typeof e=="object"&&!Array.isArray(e)}function re(){try{let e=window.localStorage.getItem(te);if(!e)return null;let t=JSON.parse(e);return!G(t)||!G(t.grades)?null:{grades:t.grades,idx:typeof t.idx=="number"?t.idx:0,orders:G(t.orders)?t.orders:null,profile:G(t.profile)?t.profile:null,submitted:t.submitted===!0}}catch{return null}}function ie(e){try{window.localStorage.setItem(te,JSON.stringify({grades:e.grades||{},idx:typeof e.idx=="number"?e.idx:0,orders:e.orders||{},profile:e.profile||null,submitted:e.submitted===!0}))}catch{}}function ne(e){let t=e.slice();for(let n=t.length-1;n>0;n--){let i=Math.floor(Math.random()*(n+1)),a=t[n];t[n]=t[i],t[i]=a}return t}function ae(e){let t={};for(let n of e)t[n.id]=ne(w);return t}function Le(e){if(!Array.isArray(e)||e.length!==w.length)return!1;let t=new Set(e);return t.size!==w.length?!1:w.every(n=>t.has(n))}function oe(e,t){let n={};for(let i of e){let a=t&&t[i.id];n[i.id]=Le(a)?a:ne(w)}return n}var Me="https://api.hsforms.com/submissions/v3/integration/submit/";function z(e,t){return t==null||t===""?null:{name:e,value:String(t)}}function De({grades:e,profile:t,dimensions:n}){let i=P(e,n),a=t||{},s=S.map(c=>{let l=i.legAverages.find(d=>d.leg===c);return z("rc_leg_"+c.toLowerCase(),l&&l.grade)});return{fields:[z("email",a.email),z("rc_overall_grade",i.overall.grade),z("rc_practical_grade",i.practical.grade),z("rc_practical_capped",i.practical.capped?"yes":"no"),...s,z("rc_redline_count",String(i.flags.length)),z("rc_redline_ids",i.flags.map(c=>c.id).join(",")),z("rc_answered",String(i.answered)),z("rc_complete",i.complete?"yes":"no"),z("rc_connections",a.connections),z("rc_employees",a.employees),z("rc_revenue_band",a.revenue),z("rc_system_type",a.systemType),z("rc_grades_json",JSON.stringify(e))].filter(Boolean),context:{pageUri:Be(),pageName:"Utility Health Report Card"}}}function Be(){try{return window.location.href}catch{return""}}function se({grades:e,profile:t,dimensions:n}){if(!F.portalId||!F.formId)return Promise.resolve({sent:!1,reason:"not-configured"});let i;try{i=JSON.stringify(De({grades:e,profile:t,dimensions:n}))}catch{return Promise.resolve({sent:!1,reason:"payload-error"})}return fetch(Me+F.portalId+"/"+F.formId,{method:"POST",headers:{"Content-Type":"application/json"},body:i}).then(a=>({sent:a.ok,status:a.status}))}function r(e,t,n){let i=document.createElement(e);return t&&(i.className=t),n!=null&&(i.textContent=n),i}function L(e){for(;e.firstChild;)e.removeChild(e.firstChild)}function Fe(e){let t=e.indexOf("\u2014");return t===-1?{letter:e.trim(),name:""}:{letter:e.slice(0,t).trim(),name:e.slice(t+1).trim()}}function Q(e,t){return!e||!t[e]?"":Fe(t[e]).name}function B(e,t){return e?e+" "+Q(e,t):"Not yet gradable"}var T={F:{fg:"#b91c1c",bg:"#fef2f2"},D:{fg:"#c2410c",bg:"#fff7ed"},C:{fg:"#b45309",bg:"#fffbeb"},B:{fg:"#4d7c0f",bg:"#f7fee7"},A:{fg:"#15803d",bg:"#f0fdf4"}};var q={T:"#0c1f30",M:"#475569",F:"#b45309"};function ce(e,{hasSaved:t,savedCount:n,total:i,onStart:a,onResume:s}){L(e);let o=r("div","zrc-landing"),c=r("div","zrc-eyebrow");c.appendChild(r("span","zrc-eyebrow-dot")),c.appendChild(r("span",null,"Free. Ungated. Nothing sold.")),o.appendChild(c),o.appendChild(r("h2","zrc-h1","An honest picture of your utility")),o.appendChild(r("p","zrc-lede","Answer 23 questions about your system and get back a plain read on where you are strong, where you are at risk, and the specific next step for each weak spot."));let l=r("div","zrc-facts");l.appendChild(J("How long it takes","About 90 to 120 minutes the first time, if your records are nearby. About 30 to 45 minutes for an annual update once the system is in routine use.")),l.appendChild(J("Who should be in the room","At minimum, the licensed operator, the board chair, and the financial clerk, together if you can manage it. Each of them sees a different part of the system. One person answering alone tends to skew the result toward whichever part they know best.")),l.appendChild(J("What happens to your answers","There is no account and no signup, and you never have to give us your email to see your score. Your answers save in this browser so you can stop and come back. When you finish, we keep a copy of the grades and the size of your system to build a picture of how small systems are really doing. That copy has no name on it unless you choose to give us your email at the end.")),o.appendChild(l);let d=r("div","zrc-actions"),p=r("div","zrc-start-wrap"),u=r("button","zrc-btn zrc-btn-primary","Start the Report Card");if(u.type="button",u.addEventListener("click",a),p.appendChild(u),t&&p.appendChild(r("p","zrc-start-subtext","Starts over from question 1 and clears your saved answers.")),d.appendChild(p),t){let f=r("button","zrc-btn zrc-btn-secondary","Resume where you left off ("+n+"/"+i+")");f.type="button",f.addEventListener("click",s),d.appendChild(f)}o.appendChild(d),e.appendChild(o)}function J(e,t){let n=r("div","zrc-fact");return n.appendChild(r("h3","zrc-fact-label",e)),n.appendChild(r("p","zrc-fact-body",t)),n}function le(e,t){let{dimensions:n,grades:i,idx:a,gradeLabels:s,rungOrders:o,onAnswer:c,onGoto:l,onExit:d,onViewResults:p}=t;L(e);let u=P(i,n),f=n[a],h=r("div","zrc-intake");h.appendChild(Ie(u,d)),h.appendChild(Te(u,f.leg)),h.appendChild(Oe(n,i,a,l)),h.appendChild(Ne(f,i,s,c,o&&o[f.id])),h.appendChild(Ge(n,a,l,p));let m=r("div","zrc-viewresults-row"),y=r("button","zrc-link-btn","See my results so far");y.type="button",y.addEventListener("click",p),m.appendChild(y),h.appendChild(m),e.appendChild(h);let R=h.querySelector(".zrc-step-title");R&&R.focus()}function de(e,{dimensions:t,grades:n}){let i=P(n,t),a=e.querySelector(".zrc-topbar-count");a&&(a.textContent=i.answered+" of "+i.total+" answered"),S.forEach(s=>{let o=i.legAverages.find(u=>u.leg===s),c=e.querySelector('.zrc-legbar[data-leg="'+s+'"]');if(!c)return;let l=o.total?Math.round(o.answered/o.total*100):0,d=c.querySelector(".zrc-legbar-fill");d&&(d.style.width=l+"%");let p=c.querySelector(".zrc-legbar-count");p&&(p.textContent=o.answered+"/"+o.total)}),t.forEach(s=>{let o=e.querySelector('.zrc-jumpbtn[data-dim-id="'+s.id+'"]');if(!o)return;let c=!!n[s.id];o.classList.toggle("zrc-jumpbtn-answered",c),o.textContent=s.id+(c?" \u2713":""),o.setAttribute("aria-label",s.id+", "+s.name+(c?", answered":", not yet answered"))})}function Ie(e,t){let n=r("div","zrc-topbar"),i=r("button","zrc-link-btn","\u2190 Save and exit");return i.type="button",i.addEventListener("click",t),n.appendChild(i),n.appendChild(r("span","zrc-topbar-count",e.answered+" of "+e.total+" answered")),n}function Te(e,t){let n=r("div","zrc-legbars");return S.forEach(i=>{let a=e.legAverages.find(p=>p.leg===i),s=a.total?Math.round(a.answered/a.total*100):0,o=r("div","zrc-legbar"+(i===t?" zrc-legbar-current":""));o.dataset.leg=i;let c=r("div","zrc-legbar-label");c.appendChild(r("span",null,k[i])),c.appendChild(r("span","zrc-legbar-count",a.answered+"/"+a.total)),o.appendChild(c);let l=r("div","zrc-legbar-track"),d=r("div","zrc-legbar-fill");d.style.width=s+"%",d.style.background=q[i],l.appendChild(d),o.appendChild(l),n.appendChild(o)}),n}function Oe(e,t,n,i){let a=r("nav","zrc-jumpnav");return a.setAttribute("aria-label","Jump to a dimension"),S.forEach(s=>{let o=r("div","zrc-jumpgroup");o.appendChild(r("span","zrc-jumpgroup-label",k[s]));let c=r("div","zrc-jumprow");e.forEach((l,d)=>{if(l.leg!==s)return;let p=!!t[l.id],u=["zrc-jumpbtn"];d===n&&u.push("zrc-jumpbtn-current"),p&&u.push("zrc-jumpbtn-answered");let f=r("button",u.join(" "),l.id+(p?" \u2713":""));f.type="button",f.dataset.dimId=l.id,f.setAttribute("aria-label",l.id+", "+l.name+(p?", answered":", not yet answered")),d===n&&f.setAttribute("aria-current","step"),f.addEventListener("click",()=>i(d)),c.appendChild(f)}),o.appendChild(c),a.appendChild(o)}),a}function Ne(e,t,n,i,a){let s=r("div","zrc-card zrc-step-card"),o=r("div","zrc-step-meta");o.appendChild(r("span","zrc-legbadge",k[e.leg]+" \xB7 "+e.id)),s.appendChild(o);let c="zrc-dim-heading",l=r("h2","zrc-step-title",e.name);l.id=c,l.tabIndex=-1,s.appendChild(l),s.appendChild(r("p","zrc-step-def",e.definition)),s.appendChild(r("div","zrc-step-instruction","Pick the description that sounds most like your utility today."));let d=r("div","zrc-ruggroup zrc-ruggroup-blind");d.setAttribute("role","radiogroup"),d.setAttribute("aria-labelledby",c);let p=a&&a.length===w.length?a:w;return p.forEach((u,f)=>{let h=r("label","zrc-rung"),m=document.createElement("input");m.type="radio",m.name="zrc-rung-"+e.id,m.value=u,m.className="zrc-rung-input",m.checked=t[e.id]===u,m.setAttribute("aria-label","Option "+(f+1)+" of "+p.length+": "+e.rungs[u]),m.addEventListener("change",()=>i(e.id,u)),h.appendChild(m);let y=r("span","zrc-rung-text");y.appendChild(r("span","zrc-rung-desc",e.rungs[u])),h.appendChild(y),d.appendChild(h)}),s.appendChild(d),s}function Ge(e,t,n,i){let a=t===e.length-1,s=r("div","zrc-navrow"),o=r("button","zrc-btn zrc-btn-quiet","\u2190 Previous");o.type="button",o.disabled=t===0,o.addEventListener("click",()=>n(t-1)),s.appendChild(o);let c=r("button","zrc-btn zrc-btn-quiet","Skip for now");c.type="button",c.addEventListener("click",()=>{a?i():n(t+1)}),s.appendChild(c);let l=r("button","zrc-btn zrc-btn-primary",a?"See my results \u2192":"Next \u2192");return l.type="button",l.addEventListener("click",()=>{a?i():n(t+1)}),s.appendChild(l),s}var qe=[["Under 500","Under 500"],["500 to 1,000","500 to 1,000"],["1,000 to 3,300","1,000 to 3,300"],["3,300 to 10,000","3,300 to 10,000"],["Over 10,000","Over 10,000"]],je=[["1 or fewer","1 or fewer"],["2 to 4","2 to 4"],["5 to 9","5 to 9"],["10 to 24","10 to 24"],["25 or more","25 or more"]],_e=[["Under $250k","Under $250k"],["$250k to $500k","$250k to $500k"],["$500k to $1M","$500k to $1M"],["$1M to $2.5M","$1M to $2.5M"],["$2.5M to $5M","$2.5M to $5M"],["Over $5M","Over $5M"],["Not sure","Not sure"]],Ue=[["Water","Water"],["Wastewater","Wastewater"],["Both","Both"]];function pe(e,t){let{profile:n,onSubmit:i,onSkip:a,onBack:s}=t;L(e);let o=n||{},c=r("div","zrc-profile"),l=r("div","zrc-topbar"),d=r("button","zrc-link-btn","\u2190 Back to the questions");d.type="button",d.addEventListener("click",s),l.appendChild(d),c.appendChild(l);let p=r("h2","zrc-h1","A few optional questions");p.tabIndex=-1,c.appendChild(p),c.appendChild(r("p","zrc-lede","Every question below is optional. Skip this and you get the exact same full report on the next screen. Nothing here changes your score.")),c.appendChild(r("p","zrc-profile-why","Here is why we ask. There is very little data anywhere on how small water and wastewater systems are actually doing day to day. Answers like yours help build a real picture of that, one that gets shared back rather than locked away. Answer as many or as few as you want."));let u=r("div","zrc-profile-grid");u.appendChild(j("zrc-profile-connections","Service connections","How many service connections does your system have?",qe,o.connections)),u.appendChild(j("zrc-profile-employees","Staff","How many full-time-equivalent staff work on the utility?",je,o.employees)),u.appendChild(j("zrc-profile-revenue","Annual operating revenue","Your best estimate is fine.",_e,o.revenue)),u.appendChild(j("zrc-profile-systemtype","System type","Water, wastewater, or both?",Ue,o.systemType)),c.appendChild(u);let f=r("div","zrc-profile-field zrc-profile-email-field"),h=r("label","zrc-profile-label","Email");h.htmlFor="zrc-profile-email",f.appendChild(h),f.appendChild(r("p","zrc-profile-help","Only if you want us to be able to reach you about your results. Not a newsletter signup. Your report is on the next screen either way, and you can print or save it from there."));let m=document.createElement("input");m.type="email",m.id="zrc-profile-email",m.className="zrc-profile-input",m.placeholder="you@utility.gov",m.value=o.email||"",f.appendChild(m),c.appendChild(f);let y={connections:u.querySelector("#zrc-profile-connections"),employees:u.querySelector("#zrc-profile-employees"),revenue:u.querySelector("#zrc-profile-revenue"),systemType:u.querySelector("#zrc-profile-systemtype")},R=r("div","zrc-profile-actions"),C=r("button","zrc-btn zrc-btn-secondary","Skip, just show me my report");C.type="button",C.addEventListener("click",a),R.appendChild(C);let v=r("button","zrc-btn zrc-btn-primary","Submit and see my report");v.type="button",v.addEventListener("click",()=>{let b={};y.connections.value&&(b.connections=y.connections.value),y.employees.value&&(b.employees=y.employees.value),y.revenue.value&&(b.revenue=y.revenue.value),y.systemType.value&&(b.systemType=y.systemType.value);let A=m.value.trim();A&&(b.email=A),i(b)}),R.appendChild(v),c.appendChild(R),e.appendChild(c),p.focus()}function j(e,t,n,i,a){let s=r("div","zrc-profile-field"),o=r("label","zrc-profile-label",t);o.htmlFor=e,s.appendChild(o),n&&s.appendChild(r("p","zrc-profile-help",n));let c=document.createElement("select");c.id=e,c.className="zrc-profile-select";let l=document.createElement("option");return l.value="",l.textContent="Prefer not to say",c.appendChild(l),i.forEach(([d,p])=>{let u=document.createElement("option");u.value=d,u.textContent=p,d===a&&(u.selected=!0),c.appendChild(u)}),s.appendChild(c),s}var He=new Set(O);function _(e,t){if(!e||!t||!e.rungs||!e.rungs[t])return null;let n=r("p","zrc-whatmeans");return n.appendChild(r("span","zrc-whatmeans-label","What this means: ")),n.appendChild(document.createTextNode(e.rungs[t])),n}function me(e){return"zrc-redline-"+e}function ge(e){return"zrc-allrow-"+e}function fe(e,t,n,i){let a=r("div",e+" zrc-see-above");a.appendChild(r("span",e.indexOf("actionplan")>-1?"zrc-actionplan-leg":"zrc-onerungup-leg",k[t]+" \xB7 "+n));let s=document.createElement("a");return s.className="zrc-see-above-link",s.href="#"+me(n),s.textContent="See "+i+" above",a.appendChild(s),a}function $e(e){return e.length<=1?e.join(""):e.length===2?e[0]+" and "+e[1]:e.slice(0,-1).join(", ")+", and "+e[e.length-1]}var Qe='<svg xmlns="http://www.w3.org/2000/svg" width="595.995" height="71.933" viewBox="0 0 595.995 71.933" fill="none"><path d="M 217.668 50.734 L 188.205 50.734 L 217.245 20.29 L 217.245 7.002 L 164.076 7.002 L 164.076 21.299 L 193.2 21.299 L 163.653 52.248 L 163.653 65.872 L 217.668 65.872 L 217.668 50.734 Z" fill="#0c1f30"></path><path d="M 242.616 65.872 L 242.616 7.002 L 224.498 7.002 L 224.498 65.872 L 242.616 65.872 Z" fill="#0c1f30"></path><path d="M 249.861 7.002 L 249.861 65.872 L 267.809 65.872 L 267.809 47.875 L 301.675 47.875 C 303.707 42.576 304.723 35.933 304.723 27.943 C 304.723 19.449 303.538 12.469 301.336 7.002 L 249.861 7.002 Z M 267.809 20.711 L 285.927 20.711 C 286.435 22.477 286.774 24.831 286.774 27.691 C 286.774 30.718 286.435 33.073 285.927 34.755 L 267.809 34.755 L 267.809 20.711 Z" fill="#0c1f30"></path><path d="M 364.67 21.804 L 364.67 7.002 L 309.808 7.002 L 309.808 21.804 L 328.265 21.804 L 328.265 65.872 L 346.213 65.872 L 346.213 21.804 L 364.67 21.804 Z" fill="#0c1f30"></path><path d="M 390.031 65.872 L 390.031 7.002 L 371.913 7.002 L 371.913 65.872 L 390.031 65.872 Z" fill="#0c1f30"></path><path d="M 415.217 51.154 L 415.217 7.002 L 397.268 7.002 L 397.268 65.872 L 442.817 65.872 L 442.817 51.154 L 415.217 51.154 Z" fill="#0c1f30"></path><path d="M 468.149 65.872 L 468.149 7.002 L 450.031 7.002 L 450.031 65.872 L 468.149 65.872 Z" fill="#0c1f30"></path><path d="M 530.249 21.801 L 530.249 7 L 475.386 7 L 475.386 21.801 L 493.843 21.801 L 493.843 65.869 L 511.792 65.869 L 511.792 21.801 L 530.249 21.801 Z" fill="#0c1f30"></path><path d="M 574.518 44.991 L 595.995 7.002 L 576.691 7.002 L 565.543 30.118 L 554.565 7.002 L 535.092 7.002 L 556.569 44.991 L 556.569 65.872 L 574.518 65.872 L 574.518 44.991 Z" fill="#0c1f30"></path><path d="M 76.447 60.312 C 74.624 62.221 75.977 65.384 78.616 65.384 L 106.198 65.384 C 107.018 65.384 107.802 65.048 108.368 64.455 L 133.902 37.709 C 135.724 35.8 134.371 32.637 131.732 32.637 L 104.15 32.637 C 103.331 32.637 102.547 32.973 101.98 33.566 L 76.447 60.312 Z" fill="#0c1f30"></path><path d="M 0.836 34.224 C -0.986 36.133 0.367 39.296 3.006 39.296 L 30.588 39.296 C 31.407 39.296 32.191 38.96 32.758 38.367 L 58.291 11.621 C 60.114 9.712 58.761 6.549 56.121 6.549 L 28.54 6.549 C 27.72 6.549 26.936 6.885 26.37 7.478 L 0.836 34.224 Z" fill="#0c1f30"></path><path d="M 118.221 5.072 C 120.043 3.163 118.69 0 116.051 0 L 81.645 0 C 80.825 0 80.041 0.335 79.475 0.928 L 16.519 66.861 C 14.696 68.77 16.049 71.933 18.688 71.933 L 53.094 71.933 C 53.914 71.933 54.698 71.598 55.264 71.005 L 118.221 5.072 Z" fill="#0c1f30"></path></svg>';function Je(){try{return new Date().toLocaleDateString("en-US",{year:"numeric",month:"long",day:"numeric"})}catch{try{return new Date().toDateString()}catch{return""}}}function Ze(){let e=r("div","zrc-print-logo");e.setAttribute("aria-hidden","true");let t=r("div","zrc-print-logo-mark");t.innerHTML=Qe,e.appendChild(t);let n=r("div","zrc-print-logo-meta");return n.appendChild(r("div","zrc-print-logo-title","Utility Health Report Card")),n.appendChild(r("div","zrc-print-logo-date",Je())),e.appendChild(n),e}function Ye(){let e=r("div","zrc-print-footer");return e.setAttribute("aria-hidden","true"),e.textContent="ziptility.com/tools/report-card",e}var Ke={F:"This system needs help now.",D:"This system is carrying real risk.",C:"This system is getting by.",B:"This system is in solid shape.",A:"This system is thriving."};function he(e,t){let{dimensions:n,grades:i,gradeLabels:a,onBack:s,onEdit:o}=t;L(e);let c={};n.forEach(m=>{c[m.id]=m});let l=P(i,n),d=new Set(l.flags.map(m=>m.id)),p=r("div","zrc-results");p.appendChild(Ze());let u=r("div","zrc-topbar zrc-noprint"),f=r("button","zrc-link-btn","\u2190 Back to the questions");f.type="button",f.addEventListener("click",s),u.appendChild(f),u.appendChild(r("span","zrc-topbar-count",l.answered+" of "+l.total+" dimensions answered")),p.appendChild(u),p.appendChild(Ve(l,a)),p.appendChild(et(n,i,a)),p.appendChild(it(n,i,l.overall.grade)),p.appendChild(at(l,c,a,n,o)),p.appendChild(ot(l,c,a,d)),p.appendChild(st(n,i,a,o,d)),p.appendChild(ct()),p.appendChild(Ye()),e.appendChild(p);let h=p.querySelector(".zrc-h1");h&&(h.tabIndex=-1,h.focus())}function Ve(e,t){let n=r("section","zrc-composite"),i=e.practical.capped,a=i?e.practical.grade:e.overall.grade;n.appendChild(r("h2","zrc-h1",a?Ke[a]:"Answer a few dimensions to see your picture")),i&&(n.appendChild(Xe(e,t)),n.appendChild(r("p","zrc-composite-secondary-label","Descriptive composite (the plain average, before the cap)")));let s=r("div","zrc-composite-grid"+(i?" zrc-composite-grid-secondary":""));return e.legAverages.forEach(o=>{s.appendChild(ue(k[o.leg],o.grade,o.answered,o.total,t))}),s.appendChild(ue("Overall",e.overall.grade,e.answered,e.total,t)),n.appendChild(s),n}function Xe(e,t){let n=r("div","zrc-practical-lead"),i=r("div","zrc-practical-grade-big"),a=T[e.practical.grade];i.style.color=a.fg,i.style.background=a.bg,i.appendChild(r("span","zrc-practical-letter",e.practical.grade)),i.appendChild(r("span","zrc-practical-label","Practical Grade")),n.appendChild(i);let s=$e(e.flags.map(c=>c.name)),o=e.flags.length+" of your critical dimensions scored F: "+s+". Your Practical Grade is "+e.practical.grade+", and that is the one to act on.";return n.appendChild(r("p","zrc-practical-explain",o)),n}function ue(e,t,n,i,a){let s=r("div","zrc-composite-card");s.appendChild(r("div","zrc-composite-card-label",e));let o=r("div","zrc-composite-card-grade");if(t){let c=T[t];o.style.color=c.fg,o.style.background=c.bg,o.appendChild(r("span","zrc-composite-letter",t)),o.appendChild(r("span","zrc-composite-name",Q(t,a)))}else o.appendChild(r("span","zrc-composite-name","Not yet gradable"));return s.appendChild(o),s.appendChild(r("div","zrc-composite-card-sub",n+" of "+i+" answered")),s}function et(e,t,n){let i=r("section","zrc-bars-section");i.appendChild(r("h2","zrc-h2","All 23 dimensions")),i.appendChild(r("p","zrc-section-lede","Grouped by Technical, Managerial, and Financial capacity. Every bar carries its letter and its name as text; colour is never the only signal.")),i.appendChild(tt(e,t));let a=r("div","zrc-bars-grid");return S.forEach(s=>{let o=r("div","zrc-bars-group"),c=r("h3","zrc-bars-group-title"),l=r("span","zrc-swatch");l.style.background=q[s],c.appendChild(l),c.appendChild(document.createTextNode(k[s]+" capacity")),o.appendChild(c),e.filter(d=>d.leg===s).forEach(d=>{o.appendChild(rt(d,t[d.id],n))}),a.appendChild(o)}),i.appendChild(a),i}function tt(e,t){let n=r("nav","zrc-jumpnav zrc-noprint");return n.setAttribute("aria-label","Jump to a dimension below"),S.forEach(i=>{let a=r("div","zrc-jumpgroup");a.appendChild(r("span","zrc-jumpgroup-label",k[i]));let s=r("div","zrc-jumprow");e.forEach(o=>{if(o.leg!==i)return;let c=!!t[o.id],l=document.createElement("a");l.className="zrc-jumpbtn"+(c?" zrc-jumpbtn-answered":""),l.href="#"+ge(o.id),l.textContent=o.id+(c?" \u2713":""),l.setAttribute("aria-label",o.id+", "+o.name),s.appendChild(l)}),a.appendChild(s),n.appendChild(a)}),n}function rt(e,t,n){let i=r("div","zrc-bar-row");i.id=ge(e.id);let a=r("div","zrc-bar-head"),s=r("span","zrc-bar-name",e.name+" ");He.has(e.id)&&s.appendChild(r("span","zrc-critical-tag","critical dimension")),a.appendChild(s),a.appendChild(r("span","zrc-bar-gradetext",B(t,n))),i.appendChild(a);let o=r("div","zrc-bar-track"),c=r("div","zrc-bar-fill"),l=t?w.indexOf(t):-1;c.style.width=l>=0?Math.round((l+1)/w.length*100)+"%":"0%",c.style.background=t?T[t].fg:"#cbd5e1",o.appendChild(c),i.appendChild(o);let d=_(e,t);d&&i.appendChild(d);let p=document.createElement("details");p.className="zrc-cite-details";let u=document.createElement("summary");return u.className="zrc-cite-summary",u.textContent="Citation",p.appendChild(u),p.appendChild(r("p","zrc-cite",e.citation)),i.appendChild(p),i}function it(e,t,n){let i=r("section","zrc-wheel-section zrc-noprint");i.appendChild(r("h2","zrc-h2","The same picture, as a wheel")),i.appendChild(r("p","zrc-section-lede","A picture only. The bars above carry the same information as text."));let a=nt(e,t,n);return i.appendChild(a),i}function nt(e,t,n){let l=e.length,p=(360-6*S.length)/l,u=(A,x)=>{let g=(x-90)*Math.PI/180;return[150+A*Math.cos(g),150+A*Math.sin(g)]},f=(A,x)=>{let[g,W]=u(128,A),[E,U]=u(128,x),[M,be]=u(58,x),[ve,we]=u(58,A),Z=x-A>180?1:0;return"M"+g+","+W+" A128,128 0 "+Z+" 1 "+E+","+U+" L"+M+","+be+" A58,58 0 "+Z+" 0 "+ve+","+we+" Z"},h="http://www.w3.org/2000/svg",m=document.createElementNS(h,"svg");m.setAttribute("viewBox","0 0 300 300"),m.setAttribute("width","260"),m.setAttribute("height","260"),m.setAttribute("aria-hidden","true"),m.setAttribute("focusable","false");let y=0,R=e.length?e[0].leg:"T";e.forEach((A,x)=>{x>0&&A.leg!==R&&(y+=6,R=A.leg);let g=y,W=y+p,E=t[A.id],U=E?T[E].fg:"#e2e8f0",M=document.createElementNS(h,"path");M.setAttribute("d",f(g,W)),M.setAttribute("fill",U),M.setAttribute("stroke","#ffffff"),M.setAttribute("stroke-width","1.5"),m.appendChild(M),y=W});let C=document.createElementNS(h,"circle");C.setAttribute("cx",String(150)),C.setAttribute("cy",String(150)),C.setAttribute("r",String(52)),C.setAttribute("fill","#fcfaf6"),m.appendChild(C);let v=document.createElementNS(h,"text");v.setAttribute("x",String(150)),v.setAttribute("y",String(152)),v.setAttribute("text-anchor","middle"),v.setAttribute("dominant-baseline","middle"),v.setAttribute("font-family","Archivo, sans-serif"),v.setAttribute("font-weight","900"),v.setAttribute("font-size","42"),v.setAttribute("fill","#0c1f30"),v.textContent=n||"",m.appendChild(v);let b=document.createElementNS(h,"text");return b.setAttribute("x",String(150)),b.setAttribute("y",String(178)),b.setAttribute("text-anchor","middle"),b.setAttribute("font-family","Geist, sans-serif"),b.setAttribute("font-size","11"),b.setAttribute("fill","#94a3b8"),b.textContent="overall",m.appendChild(b),m}function at(e,t,n,i,a){let s=r("section","zrc-redline-section");if(s.appendChild(r("h2","zrc-h2","These need attention first")),s.appendChild(r("p","zrc-section-lede","A small set of dimensions where a failing grade is treated as an emergency, regardless of the rest of the picture.")),e.flags.length===0)return s.appendChild(r("p","zrc-redline-clear","None of the critical dimensions are failing right now.")),s;let o=r("div","zrc-redline-list");return e.flags.forEach(c=>{let l=t[c.id],d=r("div","zrc-redline-card");d.id=me(c.id);let p=r("div","zrc-redline-head");if(p.appendChild(r("div","zrc-redline-leg",k[c.leg]+" \xB7 "+c.id)),p.appendChild(r("span","zrc-critical-tag","critical dimension")),d.appendChild(p),d.appendChild(r("h3","zrc-redline-name",c.name)),d.appendChild(r("div","zrc-redline-grade","Currently: "+B(c.grade,n))),l){let u=_(l,c.grade);u&&d.appendChild(u);let f=N(l,c.grade);f&&d.appendChild(r("p","zrc-action-text",f)),d.appendChild(r("p","zrc-cite",l.citation))}if(a&&i){let u=r("button","zrc-link-btn zrc-noprint","Revisit this dimension");u.type="button",u.addEventListener("click",()=>a(i.findIndex(f=>f.id===c.id))),d.appendChild(u)}o.appendChild(d)}),s.appendChild(o),s}function ot(e,t,n,i){let a=r("section","zrc-onerungup-section");if(a.appendChild(r("h2","zrc-h2","Your next move: one rung up")),a.appendChild(r("p","zrc-section-lede","The three dimensions where moving up one letter would help the most.")),e.oneRungUp.length===0)return a.appendChild(r("p",null,"Not enough is answered yet to rank a next move, or every graded dimension is already at A.")),a;let s=r("div","zrc-onerungup-list");return e.oneRungUp.forEach(o=>{if(i.has(o.id)){s.appendChild(fe("zrc-onerungup-card",o.leg,o.id,o.name));return}let c=t[o.id],l=r("div","zrc-onerungup-card");l.appendChild(r("div","zrc-onerungup-leg",k[o.leg]+" \xB7 "+o.id)),l.appendChild(r("h3",null,o.name)),l.appendChild(r("div","zrc-onerungup-transition","Currently "+B(o.current,n)+". Target: "+B(o.target,n)+"."));let d=_(c,o.current);d&&l.appendChild(d);let p=N(c,o.current);p&&l.appendChild(r("p","zrc-action-text",p)),s.appendChild(l)}),a.appendChild(s),a}function st(e,t,n,i,a){let s=r("section","zrc-actionplan-section");s.appendChild(r("h2","zrc-h2","Action plan")),s.appendChild(r("p","zrc-section-lede","Every graded dimension below A, with the specific next step and its source."));let o=e.filter(l=>{let d=t[l.id];return d&&d!=="A"});if(o.length===0)return s.appendChild(r("p",null,"Nothing here yet. Either nothing is graded below A, or nothing has been answered.")),s;let c=r("div","zrc-actionplan-list");return o.forEach(l=>{if(a.has(l.id)){c.appendChild(fe("zrc-actionplan-card",l.leg,l.id,l.name));return}let d=t[l.id],p=r("div","zrc-actionplan-card"),u=r("div","zrc-actionplan-head");u.appendChild(r("span","zrc-actionplan-leg",k[l.leg]+" \xB7 "+l.id)),u.appendChild(r("span","zrc-actionplan-grade",B(d,n))),p.appendChild(u),p.appendChild(r("h3",null,l.name));let f=_(l,d);f&&p.appendChild(f);let h=N(l,d);h&&p.appendChild(r("p","zrc-action-text",h)),p.appendChild(r("p","zrc-cite",l.citation));let m=r("button","zrc-link-btn zrc-noprint","Revisit this dimension");m.type="button",m.addEventListener("click",()=>i(e.indexOf(l))),p.appendChild(m),c.appendChild(p)}),s.appendChild(c),s}function ct(){let e=r("section","zrc-softcapture zrc-noprint");e.appendChild(r("h2","zrc-h2-sm","Bring this to your next board meeting")),e.appendChild(r("p",null,"The result is already on the screen above. If it is useful, email yourself a copy or print it. No account, no follow-up call."));let t=r("div","zrc-softcapture-row"),n=document.createElement("input");n.type="email",n.placeholder="you@utility.gov",n.className="zrc-email-input",n.setAttribute("aria-label","Your email address, optional"),t.appendChild(n);let i=r("button","zrc-btn zrc-btn-secondary","Email it to myself");i.type="button",i.addEventListener("click",()=>{try{let s=n.value.trim(),o=encodeURIComponent("My Utility Health Report Card result"),c=encodeURIComponent("Print this page (or save as PDF) for the full result.");window.location.href="mailto:"+s+"?subject="+o+"&body="+c}catch{}}),t.appendChild(i);let a=r("button","zrc-btn zrc-btn-quiet","Print this Report Card");return a.type="button",a.addEventListener("click",()=>{try{window.print()}catch{}}),t.appendChild(a),e.appendChild(t),e}function ye(){let e=document.getElementById("ziptility-report-card");if(!e||e.dataset.zipBooted)return;if(e.dataset.zipBooted="1",!document.getElementById("zrc-styles")){let g=document.createElement("style");g.id="zrc-styles",g.textContent=Y,document.head.appendChild(g)}if(!document.getElementById("zrc-fonts")){let g=document.createElement("link");g.rel="preconnect",g.href="https://fonts.googleapis.com";let W=document.createElement("link");W.rel="preconnect",W.href="https://fonts.gstatic.com",W.crossOrigin="anonymous";let E=document.createElement("link");E.id="zrc-fonts",E.rel="stylesheet",E.href="https://fonts.googleapis.com/css2?family=Archivo:wght@400;500;600;700;800;900&family=Geist:wght@400;500;600&display=swap",document.head.append(g,W,E)}window.addEventListener("beforeprint",()=>{e.querySelectorAll(".zrc-cite-details").forEach(g=>{g.dataset.zrcWasOpen=g.open?"1":"0",g.open=!0})}),window.addEventListener("afterprint",()=>{e.querySelectorAll(".zrc-cite-details").forEach(g=>{g.open=g.dataset.zrcWasOpen==="1"})});let t=H.dimensions,n=H.gradeLabels,i=null;try{i=re()}catch{i=null}let a=i&&i.grades||{},s=i&&typeof i.idx=="number"?i.idx:0,o=oe(t,i&&i.orders),c=i&&i.profile||null,l=!!(i&&i.submitted),d="landing";e.innerHTML="";let p=document.createElement("div");p.className="zrc-wrap",e.appendChild(p);function u(){try{ie({grades:a,idx:s,orders:o,profile:c,submitted:l})}catch{}}function f(g){return Math.max(0,Math.min(t.length-1,g))}function h(){d="landing",x()}function m(g){d="intake",typeof g=="number"&&(s=f(g)),u(),x()}function y(){s=0,a={},o=ae(t),m(0)}function R(){if(c||l){C();return}d="profile",x()}function C(){d="results",x()}function v(g){c=g||{},u(),se({grades:a,profile:c,dimensions:t}).then(()=>{l=!0,u()}).catch(()=>{}),C()}function b(g,W){a=Object.assign({},a,{[g]:W}),u(),de(p,{dimensions:t,grades:a})}function A(){return{dimensions:t,grades:a,idx:s,gradeLabels:n,rungOrders:o,onAnswer:b,onGoto:m,onExit:h,onViewResults:R}}function x(){if(d==="landing"){let g=P(a,t);ce(p,{hasSaved:g.answered>0,savedCount:g.answered,total:g.total,onStart:y,onResume:()=>m(s)})}else d==="intake"?le(p,A()):d==="profile"?pe(p,{profile:c,onSubmit:v,onSkip:C,onBack:()=>m(s)}):he(p,{dimensions:t,grades:a,gradeLabels:n,profile:c,onBack:()=>m(s),onEdit:g=>m(g)})}x()}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",ye):ye();})();

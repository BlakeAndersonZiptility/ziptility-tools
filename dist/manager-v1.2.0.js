/* Ziptility Manager Tools manager-v1.2.0 (02f170ad80063377d1e76879150c8eb8eb4417ec) https://github.com/BlakeAndersonZiptility/ziptility-tools */
(()=>{var F=`/* Ziptility Manager Toolbox - styles for the three manager-facing tools.
...
`}var T={hubspotPortalId:"4938013",hubspotFormId:"",fallbackEmail:"sales@ziptility.com"},O="zip-manager-energy-history";function k(e){if(e==null||!isFinite(e))return"";let t=Math.round(e*1e6)/1e6;return Math.abs(t)>=1e3?t.toLocaleString("en-US",{maximumFractionDigits:2}):String(parseFloat(t.toFixed(4)))}function L(e){if(e==null||!isFinite(e))return"";let t=e<0?"-":"",i=Math.abs(e),n=i<100?2:0;return t+"$"+i.toLocaleString("en-US",{minimumFractionDigits:n,maximumFractionDigits:n})}function Q(e){try{let t=window.localStorage.getItem(e);return t?JSON.parse(t):null}catch{return null}}function Z(e,t){try{window.localStorage.setItem(e,JSON.stringify(t))}catch{}}var G={"repair-or-replace":[{k:"breaksYr",label:"Your breaks/yr",f:k},{k:"breakEvenN",label:"Break-even breaks/yr",f:k},{k:"annualRepair",label:"Annual repair cost",f:L},{k:"annualizedReplace",label:"Annualized replacement cost",f:L}],"cost-of-turnover":[{k:"breakEvenRaise",label:"Retention-raise break-even ($/operator/yr)",f:L},{k:"costPerDeparture",label:"Cost per departure",f:L}],"energy-cost":[{k:"kwhPerKgal",label:"kWh / 1,000 gal",f:k}]},U={"repair-or-replace":e=>{let t="--",i="break-even ratio unavailable";if(e.breakEvenN>0){let n=e.breaksYr/e.breakEvenN;if(isFinite(n)){let a=Math.round(n*10)/10,d=a>1?"over":a<1?"under":"at";t=k(a)+"x",i=d+" the break-even line"}}return{numeral:t,caption:i,badge:e.verdict,badgeClass:e.verdict==="REPLACE"?"alert":e.verdict==="KEEP REPAIRING"?"good":"watch"}},"cost-of-turnover":e=>({numeral:L(e.annualCost),caption:"the annual cost of churn",badge:null,badgeClass:null}),"energy-cost":e=>{let t=e.pmTriggered&&e.pmTriggered.length>0;return{numeral:L(e.costPerKgal),caption:"per 1,000 gallons",badge:t?"PM check advised":"On track",badgeClass:t?"watch":"good"}}};function J(e){let t=[],i=null;return e.fields.forEach(n=>{let a=n.section||"";(!i||i.section!==a)&&(i={section:a,fields:[]},t.push(i)),i.fields.push(n)}),t}function V(e,t){let n=(t.units||Object.keys(z[t.unit])).map(a=>'<option value="'+a+'"'+(a===t.def?" selected":"")+">"+z[t.unit][a].label+"</option>").join("");return'<select id="'+e+'__u" data-cur="'+t.def+'" aria-label="unit">'+n+"</select>"}function Y(e,t){let i="zmt-"+e.id+"-"+t.k,n=!t.unit&&t.def!=null,a=n?' value="'+m(k(t.def))+'"':"",d='<input id="'+i+'" inputmode="decimal" autocomplete="off" spellcheck="false" placeholder="-"'+a+">",s=t.unit?'<div class="zmt-uf">'+d+V(i,t)+"</div>":d,c=n?'<div class="zmt-rot">Rule of thumb, use your own number.</div>':"";return'<div class="zmt-field" data-k="'+t.k+'"><label for="'+i+'">'+m(t.label)+"</label>"+s+c+"</div>"}function X(e){if(!e.toggle)return"";let t=e.toggle,i=t.label||t.k.charAt(0).toUpperCase()+t.k.slice(1),n="zmt-toggle-label-"+e.id+"-"+t.k;return'<div class="zmt-toggle-field"><span class="zmt-toggle-caption" id="'+n+'">'+m(i)+'</span><div class="zmt-seg" role="group" aria-labelledby="'+n+'">'+t.options.map(a=>'<button type="button" data-v="'+m(a.v)+'" aria-pressed="'+(a.v===t.def)+'">'+m(a.label)+"</button>").join("")+"</div></div>"}function _(e,t){e.innerHTML=W(t);let i=e.querySelector("#zmt-stage"),n=J(t),a=t.toggle?t.toggle.def:null,d=n.map(r=>{let o=r.fields.filter(x=>!x.advanced),l=r.fields.filter(x=>x.advanced),p=(r.section?'<h2 class="zmt-section">'+m(r.section)+"</h2>":"")+'<div class="zmt-fields">'+o.map(x=>Y(t,x)).join("")+"</div>";return l.length&&(p+='<details class="zmt-advanced"><summary>Advanced</summary><div class="zmt-fields">'+l.map(x=>Y(t,x)).join("")+"</div></details>"),p}).join("");if(i.innerHTML='<div class="zmt-card"><div class="zmt-card-head"><div class="zmt-formula">'+m(t.formula)+"</div>"+X(t)+"</div>"+d+'<div class="zmt-actions"><button class="zmt-btn zmt-btn-calc" id="zmt-calc" type="button">Calculate</button><button class="zmt-btn zmt-btn-clear" id="zmt-clear" type="button">Clear</button><button class="zmt-btn zmt-btn-print" id="zmt-print" type="button">Print / PDF</button></div><div class="zmt-msg" id="zmt-msg" aria-live="polite"></div><div class="zmt-result" id="zmt-result" aria-live="polite"></div><div class="zmt-links" id="zmt-links"></div><button class="zmt-capture-open" id="zmt-capture-open" type="button" hidden>Email or print this result</button></div>',t.links&&t.links.length){let r=e.querySelector("#zmt-links");r.innerHTML="Learn more: "+t.links.map(o=>'<a href="'+o.href+'" target="_blank" rel="noopener">'+m(o.label)+"</a>").join(" &middot; ")}let s={};t.fields.forEach(r=>{s[r.k]=e.querySelector("#zmt-"+t.id+"-"+r.k)});function c(r){if(!r)return null;let o=r.value.replace(/,/g,"").trim();if(o==="")return null;let l=parseFloat(o);return isFinite(l)?l:null}function b(r){return e.querySelector("#"+r.id+"__u")}function g(r){let o=s[r.k],l=c(o);if(l==null)return null;if(r.unit){let p=b(o);return l*z[r.unit][p.value].f}return l}function h(r,o){let l=s[r.k];if(l)if(r.unit){let p=b(l);l.value=k(o/z[r.unit][p.value].f)}else l.value=k(o)}if(t.fields.forEach(r=>{if(!r.unit)return;let o=s[r.k],l=b(o);l.addEventListener("change",()=>{let p=l.dataset.cur,x=l.value,M=c(o);M!=null&&(o.value=k(M*z[r.unit][p].f/z[r.unit][x].f)),l.dataset.cur=x})}),t.toggle&&e.querySelectorAll(".zmt-seg button").forEach(r=>r.addEventListener("click",()=>{r.dataset.v!==a&&(a=r.dataset.v,e.querySelectorAll(".zmt-seg button").forEach(o=>o.setAttribute("aria-pressed",String(o.dataset.v===a))))})),t.id==="energy-cost"){let r=Q(O);if(r){let o={kwhPrior:"kwh",gallonsPrior:"gallons",startsPerDayPrior:"startsPerDay",specificCapacityPrior:"specificCapacity"};Object.keys(o).forEach(l=>{let p=s[l];p&&!p.value&&r[o[l]]!=null&&(p.value=k(r[o[l]]))})}}let u=e.querySelector("#zmt-msg"),y=e.querySelector("#zmt-result"),v=e.querySelector("#zmt-capture-open");function S(r){if(y.innerHTML="",u.textContent="",u.className="zmt-msg",r.error){u.textContent=r.error;return}t.fields.forEach(f=>{if(f.k in r.values){let w=r.values[f.k];typeof w=="number"&&isFinite(w)&&r.computed.includes(f.k)&&(h(f,w),s[f.k]&&s[f.k].classList.add("zmt-computed"))}});let o=U[t.id],l=o?o(r.values):null,p="";if(l){let f=l.badge?'<div class="zmt-badge zmt-badge-'+l.badgeClass+'">'+m(l.badge)+"</div>":"";p+='<div class="zmt-hero">'+f+'<div class="zmt-hero-num">'+m(l.numeral)+'</div><div class="zmt-hero-label">'+m(l.caption)+"</div></div>"}if(t.interpret){let f=t.interpret(r.values);f&&(p+='<div class="zmt-insight zmt-show zmt-'+f.level+'"><span class="zmt-lead">Note</span>'+m(f.text)+"</div>")}let M=(G[t.id]||[]).map(f=>{let w=r.values[f.k];if(w==null)return"";let R=m(f.f?f.f(w):String(w));return'<div class="zmt-stat"><span class="zmt-stat-label">'+m(f.label)+'</span><span class="zmt-stat-val">'+R+"</span></div>"}).join("");if(p+='<div class="zmt-stats">'+M+"</div>",r.values.flagOutlier===!0&&(p+='<div class="zmt-chip zmt-chip-watch zmt-flag">Outlier: '+k(r.values.breaksPerMileYr)+" breaks/mi/yr (>= 2x cohort avg of "+k(r.values.cohortAvg)+")</div>"),r.values.pmTriggered&&r.values.pmTriggered.length&&r.values.pmTriggered.forEach(f=>{p+='<div class="zmt-chip zmt-chip-watch zmt-flag">'+m(f)+"</div>"}),y.innerHTML=p,t.id==="energy-cost"){let f={};["kwh","gallons","startsPerDay","specificCapacity"].forEach(R=>{r.values[R]!=null&&(f[R]=r.values[R])});let w={kwh:c(s.kwh),gallons:c(s.gallons),startsPerDay:c(s.startsPerDay),specificCapacity:c(s.specificCapacity)};Z(O,w)}v.hidden=!1,e.dataset.zmtResultSummary=ee(t,r)}function A(){let r={};t.fields.forEach(l=>{r[l.k]=g(l)}),t.toggle&&(r[t.toggle.k]=a);let o=t.solve(r);S(o)}e.querySelector("#zmt-calc").addEventListener("click",A),e.querySelector("#zmt-clear").addEventListener("click",()=>{t.fields.forEach(r=>{let o=s[r.k];o&&(o.value=!r.unit&&r.def!=null?k(r.def):"",o.classList.remove("zmt-computed"))}),t.toggle&&(a=t.toggle.def,e.querySelectorAll(".zmt-seg button").forEach(r=>r.setAttribute("aria-pressed",String(r.dataset.v===a)))),y.innerHTML="",u.textContent="",v.hidden=!0}),e.querySelector("#zmt-print").addEventListener("click",()=>{try{window.print()}catch{}}),t.fields.forEach(r=>{let o=s[r.k];o&&(o.addEventListener("keydown",l=>{l.key==="Enter"&&A()}),o.addEventListener("input",()=>o.classList.remove("zmt-computed")))})}function ee(e,t){let i=[],n=U[e.id],a=n?n(t.values):null;a&&(a.badge&&i.push("Verdict: "+a.badge),i.push(a.caption.charAt(0).toUpperCase()+a.caption.slice(1)+": "+a.numeral));let d=G[e.id]||[];return i.push(...d.map(s=>s.label+": "+(s.f?s.f(t.values[s.k]):t.values[s.k])).filter(Boolean)),e.title+" -- "+i.join("; ")}function B(e,t){let i=e.querySelector("#zmt-capture-open"),n=e.querySelector("#zmt-lead-modal");if(!i||!n)return;function a(){n.classList.add("zmt-show"),e.querySelector("#zmt-lead-form").style.display="block",e.querySelector("#zmt-lead-ok").style.display="none"}function d(){n.classList.remove("zmt-show")}i.addEventListener("click",a),e.querySelector("#zmt-lead-close").addEventListener("click",d),n.addEventListener("click",s=>{s.target===n&&d()}),e.querySelector("#zmt-lead-submit").addEventListener("click",()=>{let s=e.querySelector("#zmt-ld-name").value.trim(),c=e.querySelector("#zmt-ld-email").value.trim(),b=e.querySelector("#zmt-ld-util").value.trim();if(!c||!/.+@.+\..+/.test(c)){e.querySelector("#zmt-ld-email").focus();return}let g=e.dataset.zmtResultSummary||t.title+" result",h=u=>{e.querySelector("#zmt-lead-form").style.display="none";let y=e.querySelector("#zmt-lead-ok");y.style.display="block",u&&(e.querySelector("#zmt-lead-ok-msg").textContent=u)};if(T.hubspotPortalId&&T.hubspotFormId)fetch("https://api.hsforms.com/submissions/v3/integration/submit/"+T.hubspotPortalId+"/"+T.hubspotFormId,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({fields:[{name:"email",value:c},{name:"firstname",value:s},{name:"company",value:b},{name:"message",value:g}]})}).then(()=>h()).catch(()=>h("Saved. We'll be in touch shortly."));else{let u=encodeURIComponent("Please send this "+t.title+` result.

`+g+`

Name: `+s+`
Utility: `+b+`
Email: `+c);try{window.location.href="mailto:"+T.fallbackEmail+"?subject="+encodeURIComponent(t.title+" result")+"&body="+u}catch{}h("Opening your email app to finish the request.")}})}function $(){let e=document.getElementById("ziptility-manager-tools");if(!e||e.dataset.zipBooted)return;if(e.dataset.zipBooted="1",!document.getElementById("zip-manager-styles")){let a=document.createElement("style");a.id="zip-manager-styles",a.textContent=F,document.head.appendChild(a)}if(!document.getElementById("zip-manager-fonts")){let a=document.createElement("link");a.rel="preconnect",a.href="https://fonts.googleapis.com";let d=document.createElement("link");d.rel="preconnect",d.href="https://fonts.gstatic.com",d.crossOrigin="anonymous";let s=document.createElement("link");s.id="zip-manager-fonts",s.rel="stylesheet",s.href="https://fonts.googleapis.com/css2?family=Archivo:wght@400;500;600;700;800;900&family=Geist:wght@400;500;600&display=swap",document.head.append(a,d,s)}let t=!1,i=null;try{let a=new URLSearchParams(window.location.search);t=a.get("embed")==="app",i=a.get("tool")}catch{}if(t||(t=e.dataset.embed==="app"),i||(i=e.dataset.tool||null),t&&e.classList.add("zmt-embed-app"),e.innerHTML="",!i){e.innerHTML='<div class="zmt-error">This page is missing its tool id. Set <code>data-tool</code> on the mount div (or add <code>?tool=</code> to the URL) to one of: '+N.map(a=>a.id).join(", ")+".</div>";return}let n=N.find(a=>a.id===i);if(!n){e.innerHTML='<div class="zmt-error">"'+i+'" is not one of the manager tools. Available: '+N.map(a=>a.id).join(", ")+".</div>";return}_(e,n),B(e,n)}document.readyState==="loading"?document.addEventListener("DOMContentLoaded",$):$();})();
